...
for addonnodes in moment node-red-dashboard node-red-contrib-config node-red-contrib-grove node-red-contrib-diode node-red-contrib-bigtimer \
	...
	node-red-dashboard node-red-contrib-owntracks node-red-contrib-alexa-local node-red-contrib-amazon-echo node-red-contrib-alexa-notifyme node-red-contrib-heater-controller node-red-contrib-oauth2  node-red-contrib-cron-plus smithtek-node-red-lora @lora-contrib/node-red-contrib-payload-parser node-red-contrib-lora-packet-converter  node-red-contrib-zigbee2mqtt  node-red-contrib-postgresql   node-red-contrib-node-firebird  ; do
	echo "" 2>&1 | tee -a $LOGFILE
	echo "Installing node \"${addonnodes}\"" 2>&1 | tee -a $LOGFILE
	npm $NQUIET install --save ${addonnodes} 2>&1 | tee -a $LOGFILE
	echo "" 2>&1 | tee -a $LOGFILE
