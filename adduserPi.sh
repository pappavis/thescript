...
for gebr in michiele pi ; do
  ...
  sudo chmod 4755 /usr/bin/sudo  2>&1 | tee -a $LOGFILE
  echo "${ICyan}gebruiker $gebr aangemaakt, wachtwoord is \"$wachtwoord\". Afmelden en opnieuw aanmelden indien gewenst ${IWhite}\r\n\r\n"  2>&1 | tee -a $LOGFILE
...
echo "AddUserPi toevoeging afgerond." 2>&1 | tee -a $LOGFILE
echo ""  2>&1 | tee -a $LOGFILE
