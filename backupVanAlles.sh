...
BACKUPDIR=/mnt/usb0/rugsteun/dietpi_backup.7z
...
echo "7z a /mnt/usb0/rugsteun/dietpi_backup.7z" 2>&1 | tee -a $LOGFILE
...
for addonnodes in ~/dbs ~/.octoprint ~/.homeassistant  ~/.node-red/ ~.npm/ ~/single_chan_pkt_fwd/ ~/venv /~.local ~/Programmering/ /~.micropython   ; do
  ...
  echo "" 2>&1 | tee -a $LOGFILE
  sudo 7z a /mnt/usb0/rugsteun/dietpi_backup.7z -r   ${addonnodes}  2>&1 | tee -a $LOGFILE
